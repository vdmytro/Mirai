...
void UElimStreakProcessor::StartListening()
{
	...
	AddListenerHandle(MessageSubsystem.RegisterListener(ElimStreak::TAG_Mirai_Elimination_Message, this, &ThisClass::OnEliminationMessage));
}

void UElimStreakProcessor::OnEliminationMessage(FGameplayTag Channel, const FMiraiVerbMessage& Payload)
{
	...
	if (Payload.Instigator != Payload.Target)
	{
		if (APlayerState* InstigatorPS = Cast<APlayerState>(Payload.Instigator))
		{
			...
			{
				FMiraiVerbMessage ElimStreakMessage;
