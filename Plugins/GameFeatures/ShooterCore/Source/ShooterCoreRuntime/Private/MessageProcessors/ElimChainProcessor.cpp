...
void UElimChainProcessor::StartListening()
{
	...
	AddListenerHandle(MessageSubsystem.RegisterListener(ElimChain::TAG_Mirai_Elimination_Message, this, &ThisClass::OnEliminationMessage));
}

void UElimChainProcessor::OnEliminationMessage(FGameplayTag Channel, const FMiraiVerbMessage& Payload)
{
	...
	if (Payload.Instigator != Payload.Target)
	{
		if (APlayerState* InstigatorPS = Cast<APlayerState>(Payload.Instigator))
		{
			...
			else
			{
				...
				{
					FMiraiVerbMessage ElimChainMessage;
