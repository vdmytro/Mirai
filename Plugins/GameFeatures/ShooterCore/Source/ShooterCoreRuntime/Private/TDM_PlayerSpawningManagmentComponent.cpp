...
#include "Player/MiraiPlayerStart.h"
#include "Teams/MiraiTeamSubsystem.h"
...
AActor* UTDM_PlayerSpawningManagmentComponent::OnChoosePlayerStart(AController* Player, TArray<AMiraiPlayerStart*>& PlayerStarts)
{
	UMiraiTeamSubsystem* TeamSubsystem = GetWorld()->GetSubsystem<UMiraiTeamSubsystem>();
	...
	AMiraiGameState* GameState = GetGameStateChecked<AMiraiGameState>();

	AMiraiPlayerStart* BestPlayerStart = nullptr;
	double MaxDistance = 0;
	AMiraiPlayerStart* FallbackPlayerStart = nullptr;
	...
	for (APlayerState* PS : GameState->PlayerArray)
	{
		...
		{
			for (AMiraiPlayerStart* PlayerStart : PlayerStarts)
			{
				if (APawn* Pawn = PS->GetPawn())
				{
					...
					else if (PlayerStart->GetLocationOccupancy(Player) < EMiraiPlayerStartLocationOccupancy::Full)
					{
