...
namespace MiraiConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawDebugViewfinder(
		TEXT("Mirai.Weapon.AimAssist.DrawDebugViewfinder"),
		bDrawDebugViewfinder,
		TEXT("Should we draw a debug box for the aim assist target viewfinder?"),
		ECVF_Cheat);
}

const FMiraiAimAssistTarget* FindTarget(const TArray<FMiraiAimAssistTarget>& Targets, const UShapeComponent* TargetComponent)
{
	const FMiraiAimAssistTarget* FoundTarget = Targets.FindByPredicate(
	[&TargetComponent](const FMiraiAimAssistTarget& Target)
	{
...
void UAimAssistTargetManagerComponent::GetVisibleTargets(const FAimAssistFilter& Filter, const FAimAssistSettings& Settings, const FAimAssistOwnerViewData& OwnerData, const TArray<FMiraiAimAssistTarget>& OldTargets, OUT TArray<FMiraiAimAssistTarget>& OutNewTargets)
{
	...
	// Do a world trace on the Aim Assist channel to get any visible targets
	{
		...
		if(MiraiConsoleVariables::bDrawDebugViewfinder)
		{
	...
	// Gather targets that are in front of the player
	{
		...
		for (FAimAssistTargetOptions& AimAssistTarget : NewTargetData)
		{
			...
			const FMiraiAimAssistTarget* OldTarget = FindTarget(OldTargets, AimAssistTarget.TargetShapeComponent.Get());
			...
			FMiraiAimAssistTarget NewTarget;
	...
	{
		OutNewTargets.Sort([](const FMiraiAimAssistTarget& TargetA, const FMiraiAimAssistTarget& TargetB)
		{
	...
	{
		for (FMiraiAimAssistTarget& Target : OutNewTargets)
		{
...
bool UAimAssistTargetManagerComponent::DoesTargetPassFilter(const FAimAssistOwnerViewData& OwnerData, const FAimAssistFilter& Filter, const FAimAssistTargetOptions& Target, const float AcceptableRange) const
{
	...
	if (const ACharacter* TargetCharacter = Cast<ACharacter>(TargetOwningActor))
	{
		...
		{
			if (const AMiraiPlayerState* PS = TargetCharacter->GetPlayerState<AMiraiPlayerState>())
			{
		...
		{
			if (const UMiraiHealthComponent* HealthComponent = UMiraiHealthComponent::FindHealthComponent(TargetCharacter))
			{
...
void UAimAssistTargetManagerComponent::DetermineTargetVisibility(FMiraiAimAssistTarget& Target, const FAimAssistSettings& Settings, const FAimAssistFilter& Filter, const FAimAssistOwnerViewData& OwnerData)
{
	UWorld* World = GetWorld();
	check(World);
