...
namespace MiraiConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarEnableAimAssist(
		TEXT("Mirai.Weapon.EnableAimAssist"),
		bEnableAimAssist,
	...
	static FAutoConsoleVariableRef CVarDrawAimAssistDebug(
		TEXT("Mirai.Weapon.DrawAimAssistDebug"),
		bDrawAimAssistDebug,
...
///////////////////////////////////////////////////////////////////
// FMiraiAimAssistTarget

void FMiraiAimAssistTarget::ResetTarget()
{
...
FRotator FMiraiAimAssistTarget::GetRotationFromMovement(const FAimAssistOwnerViewData& OwnerInfo) const
{
...
float FMiraiAimAssistTarget::CalculateRotationToTarget2D(float TargetX, float TargetY, float OffsetY) const
{
	if (TargetX <= 0.0f)
	{
		return 0.0f;
...
void FAimAssistOwnerViewData::UpdateViewData(const APlayerController* PC)
{
	...
	// Set the Team ID
	if (AMiraiPlayerState* MiraiPS = PlayerController->GetPlayerState<AMiraiPlayerState>())
	{
		TeamID = MiraiPS->GetTeamId();
	}
	else
	{
		TeamID = INDEX_NONE;
...
FInputActionValue UAimAssistInputModifier::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
...
#if ENABLE_DRAW_DEBUG
	if (MiraiConsoleVariables::bDrawAimAssistDebug)
	{
	...
	if (!MiraiConsoleVariables::bEnableAimAssist)
	{
		return CurrentValue;
	}
...
void UAimAssistInputModifier::UpdateTargetData(float DeltaTime)
{
	...
	SwapTargetCaches();
	const TArray<FMiraiAimAssistTarget>& OldTargetCache = GetPreviousTargetCache();
	TArray<FMiraiAimAssistTarget>& NewTargetCache = GetCurrentTargetCache();
	...
	for (FMiraiAimAssistTarget& Target : NewTargetCache)
	{
...
const float UAimAssistInputModifier::GetSensitivtyScalar(const UMiraiSettingsShared* SharedSettings) const
{
	if (SharedSettings && SensitivityLevelTable)
	{
		const EMiraiGamepadSensitivity Sens = TargetingType == EMiraiTargetingType::Normal ? SharedSettings->GetGamepadLookSensitivityPreset() : SharedSettings->GetGamepadTargetingSensitivityPreset();
	...
	return (TargetingType == EMiraiTargetingType::Normal) ? 1.0f : 0.5f;	
}
...
FRotator UAimAssistInputModifier::UpdateRotationalVelocity(APlayerController* PC, float DeltaTime, FVector CurrentLookInputValue, FVector CurrentMoveInputValue)
{
	...
	const TArray<FMiraiAimAssistTarget>& TargetCache = GetCurrentTargetCache();

	float LookStickDeadzone = 0.25f;
	float MoveStickDeadzone = 0.25f;
	float SettingStrengthScalar = (TargetingType == EMiraiTargetingType::Normal) ? 1.0f : 0.5f;

	if (UMiraiLocalPlayer* LP = Cast<UMiraiLocalPlayer>(PC->GetLocalPlayer()))
	{
		UMiraiSettingsShared* SharedSettings = LP->GetSharedSettings();
	...
	for (const FMiraiAimAssistTarget& Target : TargetCache)
	{
		if (Target.bUnderAssistOuterReticle && Target.bIsVisible)
		{
...
void UAimAssistInputModifier::CalculateTargetStrengths(const FMiraiAimAssistTarget& Target, float& OutPullStrength, float& OutSlowStrength) const
{
	const bool bIsADS = (TargetingType == EMiraiTargetingType::ADS);
...
{
	if (!Canvas || !OwnerViewData.IsDataValid() || !MiraiConsoleVariables::bDrawAimAssistDebug)
	{
		return;
	}
