...
#include "Accolades/MiraiAccoladeHostWidget.h"

#include "DataRegistrySubsystem.h"
#include "MiraiLogChannels.h"
#include "Messages/MiraiNotificationMessage.h"
#include "Sound/SoundBase.h"
#include "TimerManager.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiAccoladeHostWidget)

class UUserWidget;

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Mirai_ShooterGame_Accolade, "Mirai.ShooterGame.Accolade");

static FName NAME_AccoladeRegistryID("Accolades");

void UMiraiAccoladeHostWidget::NativeConstruct()
{
	Super::NativeConstruct();

	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
	ListenerHandle = MessageSubsystem.RegisterListener(TAG_Mirai_AddNotification_Message, this, &ThisClass::OnNotificationMessage);
}

void UMiraiAccoladeHostWidget::NativeDestruct()
{
	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
...
void UMiraiAccoladeHostWidget::OnNotificationMessage(FGameplayTag Channel, const FMiraiNotificationMessage& Notification)
{
	if (Notification.TargetChannel == TAG_Mirai_ShooterGame_Accolade)
	{
		...
		{
			UE_LOG(LogMirai, Error, TEXT("Failed to find accolade registry for tag %s, accolades will not appear"), *Notification.PayloadTag.GetTagName().ToString());
			--AllocatedSequenceID;
		}
	}
}

void UMiraiAccoladeHostWidget::OnRegistryLoadCompleted(const FDataRegistryAcquireResult& AccoladeHandle, int32 SequenceID)
{
	if (const FMiraiAccoladeDefinitionRow* AccoladeRow = AccoladeHandle.GetItem<FMiraiAccoladeDefinitionRow>())
	{
...
void UMiraiAccoladeHostWidget::ConsiderLoadedAccolades()
{
...
void UMiraiAccoladeHostWidget::ProcessLoadedAccolade(const FPendingAccoladeEntry& Entry)
{
...
void UMiraiAccoladeHostWidget::DisplayNextAccolade()
{
...
void UMiraiAccoladeHostWidget::PopDisplayedAccolade()
{
	if (PendingAccoladeDisplays.Num() > 0)
	{
