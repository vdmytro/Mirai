...
#include "Input/MiraiInputModifiers.h"
...
class UMiraiAimSensitivityData;
class UMiraiSettingsShared;
...
struct FAimAssistOwnerViewData
{
	...
	/** The ID of the team that this owner is from. It is populated from the AMiraiPlayerState. If the owner does not have a player state, then it will be INDEX_NONE */
	int32 TeamID = INDEX_NONE;
};

/** A container for keeping the state of targets between frames that can be cached */
USTRUCT(BlueprintType)
struct FMiraiAimAssistTarget
{
	GENERATED_BODY()

	FMiraiAimAssistTarget() { ResetTarget(); }
...
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EMiraiTargetingType TargetingType = EMiraiTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UMiraiAimSensitivityData> SensitivityLevelTable = nullptr;
...
protected:
	...
	*/
	...
	void CalculateTargetStrengths(const FMiraiAimAssistTarget& Target, float& OutPullStrength, float& OutSlowStrength) const;

	FRotator GetLookRates(const FVector& LookInput);
	
	void SwapTargetCaches() { TargetCacheIndex ^= 1; }
	const TArray<FMiraiAimAssistTarget>& GetPreviousTargetCache() const	{ return ((TargetCacheIndex == 0) ? TargetCache1 : TargetCache0); }
	TArray<FMiraiAimAssistTarget>& GetPreviousTargetCache()				{ return ((TargetCacheIndex == 0) ? TargetCache1 : TargetCache0); }

	const TArray<FMiraiAimAssistTarget>& GetCurrentTargetCache() const	{ return ((TargetCacheIndex == 0) ? TargetCache0 : TargetCache1); }
	TArray<FMiraiAimAssistTarget>& GetCurrentTargetCache()				{ return ((TargetCacheIndex == 0) ? TargetCache0 : TargetCache1); }

	bool HasAnyCurrentTargets() const { return !GetCurrentTargetCache().IsEmpty(); }

	const float GetSensitivtyScalar(const UMiraiSettingsShared* SharedSettings) const;
	
	// Tracking of the current and previous frame's targets
	UPROPERTY()
	TArray<FMiraiAimAssistTarget> TargetCache0;

	UPROPERTY()
	TArray<FMiraiAimAssistTarget> TargetCache1;
