...
public class MiraiGameTarget : TargetRules
{
	public MiraiGameTarget(TargetInfo Target) : base(Target)
	{
	...
	internal static void ApplySharedMiraiTargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			MiraiGameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("MiraiGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}
