...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiGameInstance)

namespace Mirai
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarMiraiTestEncryption(
		TEXT("Mirai.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarMiraiUseDTLSEncryption(
		TEXT("Mirai.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Mirai.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarMiraiTestDTLSFingerprint(
		TEXT("Mirai.TestDTLSFingerprint"),
...
UMiraiGameInstance::UMiraiGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UMiraiGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(MiraiGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(MiraiGameplayTags::InitState_DataAvailable, false, MiraiGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(MiraiGameplayTags::InitState_DataInitialized, false, MiraiGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(MiraiGameplayTags::InitState_GameplayReady, false, MiraiGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UMiraiGameInstance::OnPreClientTravelToSession);
	}
}

void UMiraiGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
AMiraiPlayerController* UMiraiGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AMiraiPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UMiraiGameInstance::CanJoinRequestedSession() const
{
...
void UMiraiGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UMiraiLocalPlayer* LocalPlayer = Cast<UMiraiLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UMiraiGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Mirai::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Mirai::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("MiraiTest.pem");
...
void UMiraiGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UMiraiGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (Mirai::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (Mirai::bUseDTLSEncryption)
		{
