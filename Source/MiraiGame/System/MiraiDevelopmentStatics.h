...
#include "MiraiDevelopmentStatics.generated.h"
...
UCLASS()
class UMiraiDevelopmentStatics : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	// Should game logic skip directly to gameplay (skipping any match warmup / waiting for players / etc... aspects)
	// Will always return false except when playing in the editor and bTestFullGameFlowInPIE (in Mirai Developer Settings) is false
	UFUNCTION(BlueprintCallable, Category="Mirai")
	static bool ShouldSkipDirectlyToGameplay();

	// Should game logic load cosmetic backgrounds in the editor?
	// Will always return true except when playing in the editor and bSkipLoadingCosmeticBackgroundsInPIE (in Mirai Developer Settings) is true
	UFUNCTION(BlueprintCallable, Category = "Mirai", meta=(ExpandBoolAsExecs="ReturnValue"))
