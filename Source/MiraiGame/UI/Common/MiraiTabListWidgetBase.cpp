...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiTabListWidgetBase)

void UMiraiTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UMiraiTabListWidgetBase::NativeConstruct()
{
...
void UMiraiTabListWidgetBase::NativeDestruct()
{
	for (FMiraiTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UMiraiTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FMiraiTabDescriptor& OutTabInfo)
{
	const FMiraiTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FMiraiTabDescriptor& TabInfo) -> bool
	{
...
void UMiraiTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FMiraiTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UMiraiTabListWidgetBase::RegisterDynamicTab(const FMiraiTabDescriptor& TabDescriptor)
{
...
void UMiraiTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FMiraiTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UMiraiTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UMiraiTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FMiraiTabDescriptor* TabInfoPtr = nullptr;
	
	FMiraiTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UMiraiTabButtonInterface::StaticClass()))
	{
		...
		{
			IMiraiTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UMiraiTabListWidgetBase::IsFirstTabActive() const
{
...
bool UMiraiTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UMiraiTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UMiraiTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UMiraiTabListWidgetBase::SetupTabs()
{
	for (FMiraiTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
