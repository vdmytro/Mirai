...
#include "MiraiSettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "MiraiSettings"

namespace Mirai::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("MiraiErrors_UnknownMappingName", "Unknown Mapping");
}


UMiraiSettingKeyboardInput::UMiraiSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UMiraiSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return Mirai::ErrorMessages::UnknownMappingName;
}

FText UMiraiSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UMiraiSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UMiraiSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UMiraiSettingKeyboardInput::GetUserSettings() const
{
	if(UMiraiLocalPlayer* MiraiLocalPlayer = Cast<UMiraiLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = MiraiLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UMiraiSettingKeyboardInput::OnInitialized()
{
...
void UMiraiSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UMiraiSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
FText UMiraiSettingKeyboardInput::GetPrimaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::First);
}

FText UMiraiSettingKeyboardInput::GetSecondaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::Second);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UMiraiSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UMiraiSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UMiraiSettingKeyboardInput::RestoreToInitial()
{
...
bool UMiraiSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UMiraiSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UMiraiSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
