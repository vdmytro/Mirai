...
#include "MiraiSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EMiraiAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UMiraiSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UMiraiSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UMiraiSettingsShared, FOnSettingChangedEvent, UMiraiSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	UMiraiSettingsShared();
	...
	static UMiraiSettingsShared* CreateTemporarySettings(const UMiraiLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UMiraiSettingsShared* LoadOrCreateSettings(const UMiraiLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UMiraiSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UMiraiLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EMiraiAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EMiraiAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EMiraiAllowBackgroundAudioSetting AllowAudioInBackground = EMiraiAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EMiraiGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EMiraiGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EMiraiGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EMiraiGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EMiraiGamepadSensitivity GamepadLookSensitivityPreset = EMiraiGamepadSensitivity::Normal;
	UPROPERTY()
	EMiraiGamepadSensitivity GamepadTargetingSensitivityPreset = EMiraiGamepadSensitivity::Normal;
