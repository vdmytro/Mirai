...
USTRUCT()
struct FMiraiScalabilitySnapshot
{
	GENERATED_BODY()

	FMiraiScalabilitySnapshot();
 ...
 */
UCLASS()
class UMiraiSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static UMiraiSettingsLocal* Get();
...
public:
	...
	EMiraiStatDisplayMode GetPerfStatDisplayState(EMiraiDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EMiraiDisplayablePerformanceStat Stat, EMiraiStatDisplayMode DisplayMode);
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FMiraiScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FMiraiScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UMiraiSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
