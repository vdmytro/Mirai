...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiSafeZoneEditor)
...
UMiraiSafeZoneEditor::UMiraiSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UMiraiSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UMiraiSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UMiraiSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &UMiraiSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UMiraiSafeZoneEditor::HandleBackClicked);
	}
}

bool UMiraiSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UMiraiSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UMiraiSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UMiraiSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UMiraiSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UMiraiSettingsLocal::Get()->GetSafeZone());
}

void UMiraiSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		UMiraiSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
