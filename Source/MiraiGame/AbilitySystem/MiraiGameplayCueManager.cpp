...
#include "MiraiGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EMiraiEditorLoadMode
{
...
namespace MiraiGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Mirai.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via MiraiGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UMiraiGameplayCueManager::DumpGameplayCues));

	static EMiraiEditorLoadMode LoadMode = EMiraiEditorLoadMode::LoadUpfront;
}
...
UMiraiGameplayCueManager::UMiraiGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UMiraiGameplayCueManager* UMiraiGameplayCueManager::Get()
{
	UMiraiGameplayCueManager* MGCM = Cast<UMiraiGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!MGCM ) {
		UE_LOG(LogMirai, Error, TEXT("UMiraiGameplayCueManager::Get can't cast GameplayCueManager to MiraiGameplayCueManager"));
		return nullptr;
	}
...
void UMiraiGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UMiraiGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogMirai, Warning, TEXT("UMiraiGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UMiraiGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (MiraiGameplayCueManagerCvars::LoadMode)
	{
	case EMiraiEditorLoadMode::LoadUpfront:
		return true;
	case EMiraiEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EMiraiEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UMiraiGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UMiraiGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UMiraiGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UMiraiGameplayCueManager* GCM = Cast<UMiraiGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogMirai, Error, TEXT("DumpGameplayCues failed. No UMiraiGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogMirai, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogMirai, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogMirai, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogMirai, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogMirai, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogMirai, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogMirai, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogMirai, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogMirai, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogMirai, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogMirai, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogMirai, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UMiraiGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UMiraiGameplayCueManager* StrongThis = Get())
					{
...
void UMiraiGameplayCueManager::HandlePostGarbageCollect()
{
...
void UMiraiGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogMirai, Warning, TEXT("UMiraiGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UMiraiGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (MiraiGameplayCueManagerCvars::LoadMode)
	{
	case EMiraiEditorLoadMode::LoadUpfront:
...
void UMiraiGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UMiraiGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UMiraiGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UMiraiGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UMiraiGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UMiraiGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
