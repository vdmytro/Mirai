...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiContextEffectsSubsystem)
...
void UMiraiContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UMiraiContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UMiraiContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UMiraiContextEffectsLibrary* EffectLibrary : EffectsLibraries->MiraiContextEffectsLibraries)
			{
...
bool UMiraiContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UMiraiContextEffectsSettings* ProjectSettings = GetDefault<UMiraiContextEffectsSettings>())
	{
...
void UMiraiContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UMiraiContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UMiraiContextEffectsSet* EffectsLibrariesSet = NewObject<UMiraiContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UMiraiContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UMiraiContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->MiraiContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UMiraiContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
