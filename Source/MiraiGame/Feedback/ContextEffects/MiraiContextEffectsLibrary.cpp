...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiContextEffectsLibrary)


void UMiraiContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UMiraiContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UMiraiContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UMiraiContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FMiraiContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UMiraiActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FMiraiContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UMiraiActiveContextEffects* NewActiveContextEffects = NewObject<UMiraiActiveContextEffects>(this);
	...
	this->MiraiContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UMiraiContextEffectsLibrary::MiraiContextEffectLibraryLoadingComplete(
	TArray<UMiraiActiveContextEffects*> MiraiActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(MiraiActiveContextEffects);
}
