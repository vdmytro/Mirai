...
#include "AnimNotify_MiraiContextEffects.h"

#include "MiraiContextEffectsLibrary.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_MiraiContextEffects)

UAnimNotify_MiraiContextEffects::UAnimNotify_MiraiContextEffects()
{
}

void UAnimNotify_MiraiContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_MiraiContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_MiraiContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_MiraiContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> MiraiContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UMiraiContextEffectsInterface>())
			{
				// If so, add it to the Array
				MiraiContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UMiraiContextEffectsInterface>())
					{
						MiraiContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* MiraiContextEffectImplementingObject : MiraiContextEffectImplementingObjects)
			{
				if (MiraiContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IMiraiContextEffectsInterface::Execute_AnimMotionEffect(MiraiContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UMiraiContextEffectLibrary type
						if (UMiraiContextEffectsLibrary* EffectLibrary = Cast<UMiraiContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_MiraiContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_MiraiContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
