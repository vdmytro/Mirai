...
#include "MiraiCameraMode_ThirdPerson.h"
#include "Camera/MiraiCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/MiraiPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "MiraiCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(MiraiCameraMode_ThirdPerson)

namespace MiraiCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UMiraiCameraMode_ThirdPerson::UMiraiCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FMiraiPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UMiraiCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UMiraiCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UMiraiCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UMiraiCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IMiraiCameraAssistInterface* TargetControllerAssist = Cast<IMiraiCameraAssistInterface>(TargetController);

	IMiraiCameraAssistInterface* TargetActorAssist = Cast<IMiraiCameraAssistInterface>(TargetActor);
	...
	IMiraiCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IMiraiCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IMiraiCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IMiraiCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UMiraiCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IMiraiCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FMiraiPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(MiraiCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UMiraiCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UMiraiCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
