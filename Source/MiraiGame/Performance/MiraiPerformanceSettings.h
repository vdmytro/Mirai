...
#include "MiraiPerformanceSettings.generated.h"
...
USTRUCT()
struct FMiraiQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FMiraiPerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<EMiraiDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class EMiraiFramePacingMode : uint8
{
...
class UMiraiPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UMiraiPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UMiraiPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FMiraiQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EMiraiFramePacingMode FramePacingMode = EMiraiFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Mirai.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EMiraiFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Mirai Performance Settings"))
class UMiraiPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UMiraiPerformanceSettings();
...
public:
	...
	TArray<FMiraiPerformanceStatGroup> UserFacingPerformanceStats;
};
